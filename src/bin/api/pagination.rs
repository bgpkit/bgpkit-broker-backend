...
pub trait LoadPaginated<U>: Query + QueryId + QueryFragment<Pg> + LoadQuery<PgConnection, U> {
    fn load_with_pagination(self, conn: &PgConnection, page: i64, page_size: i64) -> QueryResult<(Vec<U>, i64)>;
}
...
{
    fn load_with_pagination(self, conn: &PgConnection, page: i64, page_size: i64) -> QueryResult<(Vec<U>, i64)> {
        let (records, total_pages) = self.paginate(page, page_size).load_and_count_pages::<U>(conn)?;
        Ok((records, total_pages))
    }
}
